...
ALTER TABLE `POKEMON` ADD CONSTRAINT `FK_POKEMON_TEAM` FOREIGN KEY (`TEAM_ID`, `USERNAME`) REFERENCES `TEAM`(`ID`, `USERNAME`) on delete cascade;
...
ALTER TABLE `POKEMON_MOVE` ADD CONSTRAINT `FK_POKEMON_MOVE_POKEMON` FOREIGN KEY (`POKEMON_ID`, `TEAM_ID`, `USERNAME`) REFERENCES `POKEMON`(`ID`, `TEAM_ID`, `USERNAME`) on delete cascade;
ALTER TABLE `POKEMON_MOVE` ADD CONSTRAINT `FK_POKEMON_MOVE_MOVE` FOREIGN KEY (`MOVE_ID`) REFERENCES `MOVE`(`ID`);

-- Procedures --

DELIMITER //
CREATE PROCEDURE TEAM_FREE_ID(IN USERNAME VARCHAR(12), OUT I INT)
BEGIN
	SELECT MAX(ID) + 1 INTO @R FROM TEAM WHERE TEAM.USERNAME = USERNAME;
    IF @R IS NULL THEN
		SET I = 1;
	ELSE
		SET I = @R;
...
DELIMITER //
CREATE PROCEDURE POKEMON_FREE_ID(IN USERNAME VARCHAR(12), IN TEAM_ID INT, OUT I INT)
BEGIN
	SELECT MAX(ID) + 1 INTO @R FROM POKEMON WHERE POKEMON.USERNAME = USERNAME AND POKEMON.TEAM_ID = TEAM_ID;
    IF @R IS NULL THEN
		SET I = 1;
	ELSE
		SET I = @R;
