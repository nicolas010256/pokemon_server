...
DELIMITER //
CREATE PROCEDURE TEAM_FREE_ID(OUT I INT, IN USERNAME_IN VARCHAR(12))
BEGIN
	SELECT MAX(ID) + 1 INTO I FROM TEAM WHERE USERNAME = USERNAME_IN;
END//
DELIMITER ;

DELIMITER //
CREATE PROCEDURE POKEMON_FREE_ID(OUT I INT, IN USENAME_IN VARCHAR(12), TEAM_ID_IN INT)
BEGIN
	SELECT MAX(ID) + 1 INTO I FROM POKEMON WHERE USERNAME = USERNAME_IN AND TEAM_ID = TEAM_ID_IN;
END//
DELIMITER ;
